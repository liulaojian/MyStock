...
BOOL CDlgDropOff::DoFilterKDJ(void)
{
	...
	for(int i=0;i<vecDropOffData.size();i++)
	{
		try
		{
			...
			float vr_pre_value = pStockVRData->vec_vr[vr_size - 2];
			float mavr_pre_value = pStockVRData->vec_mavr[vr_size - 2];

			float vr_now_value = pStockVRData->vec_vr[vr_size - 1];
			float mavr_now_value = pStockVRData->vec_mavr[vr_size - 1];

			bool bVrUpCross = false;

			if (vr_now_value > mavr_now_value)
			{
				if (vr_pre_value < mavr_pre_value)
					bVrUpCross = true;
			}
			...
			for (int j = 5; j >= 1; j--)
			{
				...
				double f_boll_down = pStockBOLLData->vec_down[m_boll_size - j];

				double f_price_ma5= vec_price_ma5[m_boll_size - j];
				...
				double f_ma5_temp2 = (f_value_low - f_price_ma5) / f_price_ma5;
				if (f_ma5_temp2 < 0.0)
					f_ma5_temp2 = f_ma5_temp2 * 3;
				f_ma5_per = (f_ma5_temp1 + f_ma5_temp2) + 1.0;
				*/

				if (f_price_ma5 < f_value_low)
				{
					double f_ma5_temp1 = (f_value_high - f_price_ma5) / f_price_ma5;

					double f_ma5_temp2 = (f_value_low - f_price_ma5) / f_price_ma5;
					f_ma5_per = (f_ma5_temp1 + f_ma5_temp2) / 2.0 + 1.0;

				}
				else if (f_price_ma5 > f_value_low && f_price_ma5 <= f_value_high)
				{
					f_ma5_per = 1.0 - (f_price_ma5 - f_value_low) / (f_value_high - f_value_low);
				}
				else //f_price_ma5>f_value_high
					f_ma5_per = (f_value_high - f_price_ma5) / (f_price_ma5 - f_value_low);
				...
				if (f_boll_up < f_value_min)
				{
					...
					double f_temp1_2 = ((f_value_high - f_value_max) / f_boll_up) * 2.0;

					double f_temp2 = (f_value_min - f_boll_up) / f_boll_up;
					double f_temp2_2 = (f_value_low - f_boll_up) / f_boll_up;
					
					f_boll_up_per = f_temp1 + f_temp1_2 + (f_temp2+ f_temp2_2)/2.0 + 1.0;
					*/
					
					double f_temp1 = (f_value_high - f_boll_up) / (f_value_high - f_boll_down);

					double f_temp2= (f_value_high - f_value_max) / (f_value_high - f_boll_down);

					double f_temp3=(f_value_low- f_boll_up) / (f_value_high - f_boll_down);
			...
			double f_max_rsi1 = -999999.0;
			int m_max_rsi1_index = -1;
			for (int j = rsi_size - mRsiPreNums; j < rsi_size; j++)
			{
				double f_rsi1 = pStockRSIData->vec_rsi_1_value[j];
				if (f_rsi1 > f_max_rsi1)
				{
					f_max_rsi1 = f_rsi1;
					m_max_rsi1_index = j;
				}
			}
			double f_max_rsi2 = pStockRSIData->vec_rsi_2_value[m_max_rsi1_index];
			double f_max_rsi3 = pStockRSIData->vec_rsi_3_value[m_max_rsi1_index];
			...
			rsiData.m_min_rsi_day = rsi_size - m_min_rsi1_index;

			rsiData.f_max_rsi1 = f_max_rsi1;
			rsiData.f_max_rsi2 = f_max_rsi2;
			rsiData.f_max_rsi3 = f_max_rsi3;
			rsiData.m_max_rsi_day = rsi_size - m_max_rsi1_index;
			...
			rsiData.b_vr_upcross = bVrUpCross;
			...
			rsiData.f_boll_up_per[3] = f_boll_up_per_all[3];
			rsiData.f_boll_up_per[4] = f_boll_up_per_all[4];

			rsiData.f_up_shadow_line_per[0] = f_up_shadow_line_per_all[0];
			rsiData.f_up_shadow_line_per[1] = f_up_shadow_line_per_all[1];
			rsiData.f_up_shadow_line_per[2] = f_up_shadow_line_per_all[2];
...
BOOL CDlgDropOff::DoFiterVPMFIEqu(void)
{
	RSIData mRSIData;
	for (int i = 0; i < vecRSIData.size(); i++)
	{
		...
		float fmfidif = fabs(mRSIData.mfi - mRSIData.f_max_mfi);
...
BOOL CDlgDropOff::DoFiterVPMFIEquLess75(void)
{
	RSIData mRSIData;
	for (int i = 0; i < vecRSIData.size(); i++)
	{
...
BOOL CDlgDropOff::DoFiterVPMFILowVale(int mMfiLowValue, int mMfiLowDay, bool bIsMfiCurMaxEqu, int mMfiDifCurMax)
{
	RSIData mRSIData;
	for (int i = 0; i < vecRSIData.size(); i++)
	{
		mRSIData = vecRSIData[i];
		bool bok1 = false;
		if (mRSIData.f_min_mfi <= (float)mMfiLowValue)
		{
			if (mRSIData.m_min_mfi_day <= mMfiLowDay)
			{
				...
				else
				{
					...
					float fper = fdif * 100.0 / mRSIData.mfi;
					if(fper<(float)mMfiDifCurMax)
						bok1 = true;
				}
			}
		}
...
BOOL CDlgDropOff::DoFiterVPBoll01(void)
{
	RSIData mRSIData;
	for (int i = 0; i < vecRSIData.size(); i++)
	{
...
BOOL CDlgDropOff::DoFiterVPReadyFor(void)
{
	RSIData mRSIData;
	for (int i = 0; i < vecRSIData.size(); i++)
	{
...
void CDlgDropOff::OnBnClickedBtnVpSel()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		else if (mSFSel == 2) //MFI<75相等
		{
			...
			DoFiterVPMFIEquLess75();

		}
		else if (mSFSel == 3)  //MFI低值设置
		{

			CDlgVPLowSet dlg;
			if (dlg.DoModal() == IDOK)
			{
				int mMfiLowValue = dlg.GetMinMfiValue();
				int mMfiLowDay = dlg.GetMinMfiDay();

				bool bIsMfiCurMaxEqu = dlg.IsMfiCurMaxEqu();
				int mMfiDifCurMax = dlg.GetMfiDifCurMax();
